...
def plot_solution(solution: dict, instance, title: str = "VRP Solution", output_path: str = "solution.png") -> None:
  ...
  depot = customers[0];
  ax.plot(depot.x_coordinate, depot.y_coordinate, "*", color="black", markersize=14, zorder=4, label="Depot");

  for c in customers[1:]:
    ax.plot(c.x_coordinate, c.y_coordinate, "o", color="steelblue", markersize=8, zorder=3);
    ax.annotate(str(c.customer_id), (c.x_coordinate, c.y_coordinate), textcoords="offset points", xytext=(5, 5), fontsize=8, zorder=5);

  patches = [mpatches.Patch(color=colours[i % len(colours)], label=f"Route {i + 1}") for i in range(len(routes))];
